...
- Overview 桌面 widget 必须给 `liquid-card` 阴影预留 bleed 空间；卡片不能贴着 scroll container 边缘，否则玻璃阴影会被外层 `overflow-y-auto` 裁切，形成可见的 section 边界。shell 的 overview scroll container 应铺满 viewport，内容宽度由 overview section 自己约束，避免在 `max-w-*` scrollport 边缘裁切阴影。
