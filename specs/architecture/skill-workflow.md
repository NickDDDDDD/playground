...
- 同一个小问题或同一轮 UI debug 应先本地迭代验证，不在每个微调后立刻 commit/push。
- 当用户仍在持续反馈同一问题，例如“还是有”“不对”“再 debug 一下”，说明该问题尚未收束；默认继续本地修复，不提交中间状态。
- 一个 commit 应对应一个稳定完成点：完整 bugfix、明确 spec/ADR 变更、可独立 review 的功能切片，或用户明确要求保存的检查点。
- push 通常跟随已确认的稳定提交；不要用远端历史记录替代本地迭代过程。
